...
<body>
    ...
    <div class="main-container">
        ...
        <div id="checkboxListContainer" style="max-height: 100px; overflow-y: auto;">
        <div id="checkboxList">
    ...
    <script>
        ...
        function updateCheckboxList(data, topN) {
            ...
            currentTopKeywords.forEach(([keyword, totalFrequency]) => {
                ...
                label.textContent = `${keyword} (${totalFrequency}) [${filteredFrequency}]`;
        ...
        function downloadCSV() {
            ...
            const csvContent = transformedData.map(row => row.join(',')).join('\n');
    ...
    for (let i = 1; i < data.length; i++) {
        const year = parseInt(data[i][0]); // Ensure year is parsed as an integer
        const keywords = data[i][1];

        // Split keywords into an array and trim whitespace
        const keywordArray = keywords.split(';').map(keyword => keyword.trim().toLowerCase());

        // Populate keywordFrequencyByYear and totalKeywordFrequency
        if (!keywordFrequencyByYear[year]) {
            keywordFrequencyByYear[year] = {};
        }
        keywordArray.forEach(keyword => {
    ...
    for (const year in keywordFrequencyByYear) {
        ...
        sortedKeywords.forEach(([keyword, frequency]) => {
            transformedData.push([keyword, parseInt(year), frequency, totalKeywordFrequency[keyword]]);
        });
    }
